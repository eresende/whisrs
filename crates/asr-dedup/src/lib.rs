...
//! multiple times. This crate provides two complementary strategies to
//! eliminate those duplicates:
//!
//! ## Strategy 1 — Timestamp-based dedup
//!
//! Use this when your ASR provider returns **per-word timestamps** (start/end
//! time for each word). Call [`DedupTracker::advance_offset`] before each chunk
//! to tell the tracker how much audio has been sent, then pass the words
//! through [`DedupTracker::filter_words`]. Words whose adjusted start time
...
//!
//! ## Strategy 2 — Text-based anchor dedup
//!
...
//! [`DedupTracker::filter_text`] with each window's complete output. The
//! tracker takes the last N words of the previous window as an "anchor",
//! searches for that anchor anywhere in the new window's text, and returns
...
//! ## Quick comparison
//!
//! |                 | Timestamp strat       | Text anchor strat  |
//! |-----------------|-----------------------|--------------------|
